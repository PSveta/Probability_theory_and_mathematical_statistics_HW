"""
Задача 1. Даны значения зарплат из выборки выпускников: 100, 80, 75, 77, 89,
33, 45, 25, 65, 17, 30, 24, 57, 55, 70, 75, 65, 84, 90, 150. Посчитать
...
# Среднее арифметическое (mean) можно вычислить, суммируя все значения массива
# и деля их на количество элементов в массиве.
...
# Среднее квадратичное отклонение(standard deviation) можно вычислить,
# используя формулу: σ = √ ( ( ∑ ( xi - μ )² ) / N )

stddev = math.sqrt(sum((x - mean) ** 2 for x in salaries) / len(salaries))
print(f'\nCреднее квадратичное отклонение = {stddev: .4f}')

# Смещенная оценка дисперсии можно вычислить, используя формулу:
# σ² = (∑ (xi - μ)²) / N
var = sum((x - mean) ** 2 for x in salaries) / len(salaries)
print('\nСмещенная оценка дисперсии = ', var)

# Несмещенная оценка дисперсии можно вычислить, используя формулу:
# σ² = ( ∑ ( xi - μ )² ) / (N-1)
